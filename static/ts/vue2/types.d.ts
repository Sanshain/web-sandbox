...
declare function computed<T>(getter: ComputedGetter$1<T>, debugOptions?: DebuggerOptions): ComputedRef<T>;
declare function computed<T>(
  options: WritableComputedOptions$1<T>,
  debugOptions?: DebuggerOptions
): WritableComputedRef<T>;
declare type ComputedGetter$1<T> = (...args: any[]) => T;
...
declare type CustomRefFactory<T> = (
  track: () => void,
  trigger: () => void
) => {
  get: () => T;
...
declare type DebuggerEvent = {} & DebuggerEventExtraInfo;
...
declare interface DebuggerOptions {
...
declare type DeepReadonly<T> = T extends Builtin
  ...
  : Readonly<T>;
declare function del<T>(array: T[], key: number): void;
declare function del(object: object, key: string | number): void;
...
declare type ErrorCapturedHook$1<TError = unknown> = (
  err: TError,
  instance: any,
  info: string
) => boolean | void;
declare function getCurrentScope(): EffectScope | undefined;
declare type IfAny$1<T, Y, N> = 0 extends 1 & T ? Y : N;
...
declare function markRaw<T extends object>(
  value: T
): T & {
  [RawSymbol]?: true;
};
...
declare function onErrorCaptured<TError = Error>(hook: ErrorCapturedHook$1<TError>, target?: any): void;
declare const onMounted: (fn: () => void, target?: any) => void;
declare const onRenderTracked: (fn: (e: DebuggerEvent) => any, target?: any) => void;
declare const onRenderTriggered: (fn: (e: DebuggerEvent) => any, target?: any) => void;
...
declare function set<T>(array: T[], key: number, value: T): T;
declare function set<T>(object: object, key: string | number, value: T): T;
...
declare type ToRef<T> = IfAny$1<T, Ref<T>, [T] extends [Ref] ? T : Ref<T>>;
...
declare type UnwrapRef<T> = T extends ShallowRef<infer V>
  ? V
...
declare function useCssVars(
  getter: (vm: Record<string, any>, setupProxy: Record<string, any>) => Record<string, string>
): void;
declare const version: string;
declare function watch<T extends MultiWatchSources, Immediate extends Readonly<boolean> = false>(
  sources: [...T],
  cb: WatchCallback<MapSources<T, false>, MapSources<T, Immediate>>,
  options?: WatchOptions$1<Immediate>
): WatchStopHandle;
...
declare interface WatchOptions$1<Immediate = boolean> extends WatchOptionsBase {
  immediate?: Immediate;
  deep?: boolean;
}
declare interface WatchOptionsBase extends DebuggerOptions {
  flush?: "pre" | "post" | "sync";
}
...
declare interface WritableComputedOptions$1<T> {
...
type ComputedOptions$1 = Record<string, ComputedGetter<any> | WritableComputedOptions<any>>;
interface MethodOptions {
  [key: string]: Function;
}
type SetupFunction<Props, RawBindings = {}, Emits extends EmitsOptions = {}> = (
  this: void,
  props: Readonly<Props>,
  ctx: SetupContext<Emits>
) => RawBindings | (() => VNode | null) | void;
type ExtractOptionProp<T> = T extends ComponentOptionsBase<infer P, any, any, any, any, any, any, any, any>
  ? unknown extends P
    ? {}
    : P
  : {};
...
> = ComponentOptionsBase<Props, RawBindings, D, C, M, Mixin, Extends, Emits, EmitsNames, Defaults> & {
  props?: PropsOptions;
} & ThisType<CreateComponentPublicInstance<Props, RawBindings, D, C, M, Mixin, Extends, Emits>>;
...
> = ComponentOptionsBase<Props, RawBindings, D, C, M, Mixin, Extends, Emits, EmitsNames, {}> & {
  props?: PropNames[];
} & ThisType<CreateComponentPublicInstance<Props, RawBindings, D, C, M, Mixin, Extends, Emits>>;
...
type CreateComponentPublicInstance<
  ...
  PublicC extends ComputedOptions$1 = UnwrapMixinsType<PublicMixin, "C"> & EnsureNonVoid<C>,
...
> = Vue3Instance<D, P, PublicProps, E, Defaults, MakeDefaultsOptional, Options> &
  Readonly<P> &
  ShallowUnwrapRef<B> &
  UnwrapNestedRefs<D> &
...
type ComponentPublicInstanceConstructor<
  T extends ComponentPublicInstance<Props, RawBindings, D, C, M> = ComponentPublicInstance<any, any, any>,
  Props = any,
  RawBindings = any,
  D = any,
...
type CreateElement$1 = CreateElement;
type RenderContext$1<Props = DefaultProps> = RenderContext<Props>;
...
> = ComponentPublicInstanceConstructor<
  CreateComponentPublicInstance<Props, RawBindings, D, C, M, Mixin, Extends, E, Props, Defaults, true> & Props
> &
...
>(options: {
  functional: true;
  props?: PropNames[];
  render?: (h: CreateElement$1, context: RenderContext$1<Props>) => any;
...
type AsyncComponentPromise<
  ...
  SetupBindings = {}
> = (
  resolve: (component: Component<Data, Methods, Computed, Props, SetupBindings>) => void,
  reject: (reason?: any) => void
) => Promise<ImportedComponent<Data, Methods, Computed, Props, SetupBindings>> | void;
...
> = object &
  ComponentOptions<
    ...
    Extends
  > &
  ThisType<CombinedVueInstance<V, Data, Methods, Computed, Readonly<Props>, SetupBindings, Mixin, Extends>>;
...
> {
  ...
  watch?: Record<
    string,
    WatchOptionsWithHandler<any> | WatchHandler<any> | Array<WatchOptionsWithHandler<any> | WatchHandler<any>>
  >;
  setup?: (
    this: void,
    props: Props,
...
type DirectiveFunction = (el: HTMLElement, binding: DirectiveBinding, vnode: VNode, oldVnode: VNode) => void;
...
type ExtendedVue<
  ...
  Extends extends ComponentOptionsMixin = ComponentOptionsMixin
> = VueConstructor<
  CombinedVueInstance<Instance, Data, Methods, Computed, Props, SetupBindings, Mixin, Extends> & Vue
>;
...
declare function getCurrentInstance(): { proxy: Vue } | null;
declare const h: CreateElement;
declare function useSlots(): SetupContext["slots"];
declare function useAttrs(): SetupContext["attrs"];
declare function useListeners(): SetupContext["listeners"];

declare function defineProps<PropNames extends string = string>(
  props: PropNames[]
): Readonly<{ [key in PropNames]?: any }>;
declare function defineProps<PP extends ComponentObjectPropsOptions = ComponentObjectPropsOptions>(
  props: PP
...
declare function defineExpose<Exposed extends Record<string, any> = Record<string, any>>(
  exposed?: Exposed
): void;
...
type InferDefault<P, T> = T extends null | number | string | boolean | symbol | Function
  ? T | ((props: P) => T)
  : (props: P) => T;
...
type AsyncComponentResolveResult<T = Component> = T | { default: T };
...
export  {
  AsyncComponent,
  Component,
  ComputedOptions$1 as ComponentComputedOptions,
  ComponentCustomOptions,
  ComponentCustomProperties,
  ...
  ComponentPublicInstance,
  ComputedGetter$1 as ComputedGetter,
  ComputedOptions,
  ComputedRef,
  ComputedSetter$1 as ComputedSetter,
  ...
  DefineComponent,
  Directive,
  DirectiveBinding$1 as DirectiveBinding,
  DirectiveFunction,
  DirectiveHook,
  DirectiveModifiers,
  DirectiveOptions,
  EffectScope,
  ErrorCapturedHook$1 as ErrorCapturedHook,
  ...
  WatchSource,
  WatchStopHandle,
  WritableComputedOptions$1 as WritableComputedOptions,
  WritableComputedRef,
  computed,
  customRef,
  Vue as default,
