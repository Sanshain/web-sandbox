...
import { compilersSet, defaultValues, singleFileEnv, playgroundObject, compilerNames } from "./features/compiler"
...
export default function initializeEditor(ace, editorOptions, values) {
   ...
   let editors = [].slice.call(document.querySelectorAll(".editor"))
      .map((/** @type {{ id: any; }} */ element, /** @type {number} */ i, /** @type {any[]} */ arr) => {
         ...
         editor.textInput
            ...
            .addEventListener(
               "keydown",
               function (/** @type {{ ctrlKey: any; keyCode: number; key: string; preventDefault: () => void; }} */ event) {
                  // console.log(event);
                  ...
                  getDocTooltip: function (/** @type {{ docHTML: string; caption: string; }} */ item) {
                     ...
                     if (!item.docHTML || (item["meta"] === "function" && domFuncs[item.caption] && domFuncs[item.caption].sign)) {
                        ...
                        if (hint) {
                           ...
                           item.docHTML =
                              "<h5>" + (hint.origin || item.caption) + "(" + args + ") : " + hint["return"] + "</h5><hr>"
                     ...
                     if (token.type == "identifier") {
                        ...
                        if (newValue !== token.value) {
                           if (newValue && newValue.match(/^[\w_][\w_\d]*$/m)) {
                              ...
                              if (threshold) {
                                 const pattern =
                                    "import \\{[wd_\\. ,]*" + token.value + "[wd_\\. ,]*\\} from ['\"]\\./([\\w\\d_\\.]+)"
